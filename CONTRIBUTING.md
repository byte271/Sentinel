...
For the Python SDK and LangChain plugin (in `python/`):
...
- **Python:** [pytest](https://pytest.org/) under `python/`. The unit suite uses a
  self-contained fake Shield (`conftest.py`) and needs no Node;
  `test_integration.py` drives the real Node Shield and is skipped automatically
  when `node` or the compiled `dist/` build is absent.
