...
//   - owasp-asi    : OWASP ASI Top-10 coverage dashboard
//   - nist-ai-rmf  : NIST AI RMF 1.0 profile
//   - eu-ai-act    : EU AI Act (2024/1689) conformity report (v0.3.0)
//
// Output as a text dashboard, JSON (for pipelines / API), or Markdown (which a
// downstream tool such as pandoc/weasyprint can render to PDF).
//
...
<<<<<<< HEAD
    if (framework === 'eu-ai-act' || framework === 'eu') {
      ...
      if (opts.format === 'markdown') return emit(EuAiActAssessor.renderMarkdown(report), opts.output, 'EU AI Act report');
      // text summary
      const text = [
        `EU AI Act Compliance — Annex IV score ${report.annexIVScore}% (readiness: ${report.readiness})`,
        `Risk tier: ${report.riskTier}`,
        `Days until enforcement: ${report.daysUntilEnforcement}`,
        `Gaps: ${report.gaps.length}`,
      ].join('\n');
      return emit(text, opts.output, 'EU AI Act report');
    }
